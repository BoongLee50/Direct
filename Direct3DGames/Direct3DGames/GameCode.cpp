...
void GameCode::OnUpdate()
{
	...
	if (GetAsyncKeyState('A') < 0)  // 주인공 미사일 발사
	{
			...
			if (dwCurTime - m_Enemy[i].dwOldFireTime >= m_EnemyProperty.dwFireTime)
			{
				...
				for (j = 0; j < 100; j++)
				{
					if (!m_EnemyBullet[j].nLife)
					{
						...
						m_EnemyBullet[j].vPos.z -= 0.5f; //약간 앞에서 미사일이 발사가 되도록 하기 위해									
						break; //미사일 하나만 살리는 부분의 끝, 겹치지 않게 
					}
				}
			}
		}
	...
	for (i = 0; i < 100; i++)
	{
		if (m_EnemyBullet[i].nLife > 0)
		{
			...
			if (m_EnemyBullet[i].vPos.z <= -20.0f) //경계 영역
				m_EnemyBullet[i].nLife = 0;
			else
